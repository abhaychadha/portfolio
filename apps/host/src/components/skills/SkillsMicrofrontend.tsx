...
const SKILLS_REMOTE_URL = process.env.NEXT_PUBLIC_SKILLS_REMOTE_URL
  ?? (process.env.NODE_ENV === "development" ? "http://localhost:4001" : "");

const SkillsMicrofrontend: FC = () => {
  const isRemoteConfigured = Boolean(SKILLS_REMOTE_URL);
    ...
    >
      ...
      {!isRemoteConfigured ? (
        <div className="rounded-[12px] border border-neutral-dark-gray bg-neutral-gray/30 p-6">
          <p className="font-manrope text-neutral-offwhite">
            Skills remote is not configured. Set <code>NEXT_PUBLIC_SKILLS_REMOTE_URL</code>.
          </p>
        </div>
      ) : (
        <RemoteModuleLoader
